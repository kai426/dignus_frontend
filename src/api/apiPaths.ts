...
export const API_PATHS = {
  AUTH: {
    ...
    GET_PROGRESS: (candidateId: string) =>
      `/api/auth/progress/${candidateId}`, // <- Endpoint V1 legado
  },
  ...
  CANDIDATE_AUTH: {
    ...
    LOCKOUT_STATUS: (cpf: string) =>
      `/api/candidate-auth/lockout-status/${cpf}`, // GET
  },
  // Consentimento LGPD
  CONSENT: {
    GET_STATUS: (cpf: string) => `/api/consent/status/${cpf}`, // GET
    SUBMIT: '/api/consent', // POST { hasAccepted: true }
  ...
  TESTS_V2: {
    ...
    GET_BY_ID: (testId: string, candidateId: string) =>
      `/api/v2/tests/${testId}?candidateId=${candidateId}`, // GET
    GET_ALL_FOR_CANDIDATE: (candidateId: string, testType?: TestType) =>
      `/api/v2/tests/candidate/${candidateId}${
        testType !== undefined ? `?testType=${testType}` : ''
      }`, // GET
    START: (testId: string, candidateId: string) =>
      `/api/v2/tests/${testId}/start?candidateId=${candidateId}`, // POST
    ...
    GET_QUESTIONS: (testId: string, candidateId: string) =>
      `/api/v2/tests/${testId}/questions?candidateId=${candidateId}`, // GET
    GET_STATUS: (testId: string, candidateId: string) =>
      `/api/v2/tests/${testId}/status?candidateId=${candidateId}`, // GET
    CAN_START: (candidateId: string, testType: TestType) =>
      `/api/v2/tests/candidate/${candidateId}/can-start/${testType}`, // GET
    ...
    GET_TEST_VIDEOS: (testId: string, candidateId: string) =>
      `/api/v2/tests/${testId}/videos?candidateId=${candidateId}`, // GET
    GET_VIDEO_RESPONSE: (
      testId: string,
      videoId: string,
      candidateId: string,
    ) => `/api/v2/tests/${testId}/videos/${videoId}?candidateId=${candidateId}`, // GET
    GET_SECURE_VIDEO_URL: (
      ...
      `/api/v2/tests/${testId}/videos/${videoId}/url?candidateId=${candidateId}${
        expirationMinutes ? `&expirationMinutes=${expirationMinutes}` : ''
      }`, // GET
    DELETE_VIDEO: (
      testId: string,
      videoId: string,
      ...
      `/api/v2/tests/${testId}/videos/${videoId}?candidateId=${candidateId}`, // DELETE

    // Respostas de Múltipla Escolha (para testes de Psicologia, Retenção Visual)
    SUBMIT_ANSWERS: (testId: string, candidateId: string) =>
      `/api/v2/tests/${testId}/answers?candidateId=${candidateId}`, // POST [{ questionSnapshotId, selectedAnswers, responseTimeMs? }]
    GET_TEST_ANSWERS: (testId: string, candidateId: string) =>
      `/api/v2/tests/${testId}/answers?candidateId=${candidateId}`, // GET
    UPDATE_ANSWER: (
      testId: string,
      responseId: string,
      candidateId: string,
    ) =>
      `/api/v2/tests/${testId}/answers/${responseId}?candidateId=${candidateId}`, // PUT { questionSnapshotId, selectedAnswers, responseTimeMs? }
    DELETE_ANSWER: (
      testId: string,
      responseId: string,
      candidateId: string,
    ) =>
      `/api/v2/tests/${testId}/answers/${responseId}?candidateId=${candidateId}`, // DELETE
  },
  QUESTIONNAIRE: {
...
export const CONSENT = API_PATHS.CONSENT.SUBMIT
export const CONSENT_STATUS = API_PATHS.CONSENT.GET_STATUS